...
document.getElementById("currentPage_btn_right").addEventListener("click", async function () {
    ...
    if (currentValue >= parseInt(getAfterSlashValue())) {
        ...
        try {
            ...
            const response = await axios.get(`blog/showPanigation?page=${nextPage}`);
...
document.getElementById("currentPage_btn_left").addEventListener("click", async function () {
    const currentValue = parseInt(getBeforeSlashValue());
    let prevPage;
    ...
    if (currentValue <= 1) {
        ...
        try {
            ...
            const response = await axios.get(`blog/showPanigation?page=${prevPage}`);
            renderBlogs(response.data.blogs);
